mmm <- function (formula, id, data = NULL, correlation = NULL, initEstim = NULL,
                 tol = 0.001, maxiter = 25, family = "gaussian",
                 corStruct = "independence", Mv = 1, silent = TRUE,
  ...
  formula2 <- as.formula(paste("resp ~ -1+",
                               paste(colnames(cov3),
                                     collapse = "+")))
  #library(gee)
  fit <- gee(formula = formula2, id = id5, data = cov3, R = correlation,
             b = initEstim, tol = tol, maxiter = maxiter,

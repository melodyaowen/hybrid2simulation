...
gen_crt_coprimary_data_bin <- function(K, # Number of clusters in treatment arm
  ...
  mu1_k_1 <- theta11 <- exp(beta01 + beta1)/(exp(beta01 + beta1) + 1) # Cluster K, treatment 1

  # Outcome 2 mean
  mu2_k_0 <- theta02 <- exp(beta02)/(exp(beta02) + 1)
  mu2_k_1 <- theta12 <- exp(beta02 + beta2)/(exp(beta02 + beta2) + 1)

  # V matrix for each outcome for each treatment group
  V1_k_0 <- mu1_k_0*(1-mu1_k_0)*diag(1, nrow = m, ncol = m)
  V1_k_1 <- mu1_k_1*(1-mu1_k_1)*diag(1, nrow = m, ncol = m)
  V2_k_0 <- mu2_k_0*(1-mu2_k_0)*diag(1, nrow = m, ncol = m)
  V2_k_1 <- mu2_k_1*(1-mu2_k_1)*diag(1, nrow = m, ncol = m)
